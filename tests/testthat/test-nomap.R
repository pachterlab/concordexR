...
test_that("calculateNomap (dgCMatrix) returns correct output", {
    res <- calculateNomap(sp_mtx, labels)
    expect_type(res, "list")
    expect_named(res, nms)
...
test_that("Error and messages for wrong number of labels", {
    expect_error(
        calculateNomap(sp_mtx, labels = labels1, k = knn),
        "Must have at least 2 distinct labels"
    )
    expect_error(
        calculateNomap(sp_mtx, labels = labels[1:99], k = knn),
        "Too few labels supplied"
    )
    expect_error(
        calculateNomap(sp_mtx, labels = c(labels, labels[1]), k = knn),
...
test_that("Matrix (re)-orientation is working properly", {
    ...
    # can't be oriented
    expect_error(
        calculateNomap(sp_mtx, labels, k = knn - 1),
        "Cannot determine"
    )
})
...
test_that("`calculateNomap` identifies self-referential graphs", {
    expect_warning(
        calculateNomap(mtx_self, labels, k = knn),
